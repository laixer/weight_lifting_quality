...
```{r}
create_sliding_window_summary_fn <- function(window_size) {
  return(function(df) {
    df <- mutate(df, raw_timestamp = ((raw_timestamp_part_1 - min(raw_timestamp_part_1)) * 1000000 + raw_timestamp_part_2) / 1000000.0)
    sm = list()
    for(x in seq(min(df$raw_timestamp), max(df$raw_timestamp), window_size)) {
...
for (x in seq(0.5, 5.0, 0.5)) {
  print(paste("Proccessing window size", x))
  z[[length(z) + 1]] <- rbind_all(dlply(training, .(user_name, classe), create_sliding_window_summary_fn(x)))
}
```
...
fit = list()
for (x in z) {
  fit[[length(fit) + 1]] <- train(classe ~ ., data = x, method = "rpart")
}
```