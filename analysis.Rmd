...
fit = list()
for (x in names(z)) {
  print(paste("Fitting", x))
  fit[[x]] <- train(classe ~ ., data = z[[x]], method = "rf", trControl = fitControl)
}
```
...
# fit_nn = list()
# for (x in names(z)) {
#   print(paste("Fitting nn", x))
#   fit_nn[[x]] <- train(classe ~ ., data = z[[x]], method = "nnet", 
#                        trControl = fitControlNn, trace = FALSE, MaxNWts = 10000)
# }

my.grid <- expand.grid(.decay = c(0, 1e-4, 1e-01), .size = c(5, 10, 20))

fit_nn2 <- mclapply(seq_along(z), function(i) {
  df <- z[[i]]
  print(paste("Fitting nn", names(z)[[i]]))
  fit_nn[[x]] <- train(classe ~ ., data = df, method = "nnet", 
                       trControl = fitControlNn, trace = FALSE, 
                       tuneGrid = my.grid,
                       MaxNWts = 10000)
}, mc.cores = 8)
```
...
```{r}
accuracies <- data.frame(sapply(fit, function(f) max(f$results$Accuracy)))
accuracies$window <- rownames(accuracies)
...
```{r}
accuracies <- data.frame(sapply(fit_nn2, function(f) max(f$results$Accuracy, na.rm = TRUE)))
accuracies$window <- rownames(accuracies)
names(accuracies)[[1]] <- "accuracy"
