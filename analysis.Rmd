...
fit_rf <- mclapply(seq_along(z), function(i) {
  ...
  return(train(classe ~ ., data = z[[x]], method = "rf", trControl = fitControlRf))
}, mc.cores = 4)
```

```{r}
fitControlNn <- trainControl(method = "cv", number = 10)
my.grid <- expand.grid(.decay = c(0), .size = c(20))
...
```{r}
accuracies <- data.frame(sapply(fit_rf, function(f) max(f$results$Accuracy)))
...
```{r}
accuracies <- data.frame(sapply(fit_nn, function(f) max(f$results$Accuracy, na.rm = TRUE)))
accuracies$window <- rownames(accuracies)
names(accuracies)[[1]] <- "accuracy"
