...
summarize_window <- function(df) {
  ...
  for (f in c(belt_features, arm_features, dumbbell_features, forearm_features)) {
    ndf[,paste("max", f, sep="_")] <- max(df[,f])
    ndf[,paste("min", f, sep="_")] <- min(df[,f])
    ndf[,paste("avg", f, sep="_")] <- mean(df[,f])
    #ndf[,paste("var", f, sep="_")] <- var(df[,f])
  }
  return(ndf)
}
...
create_sliding_window_summary_fn <- function(window_size, overlap) {
  return(function(df) {
    ...
    for(x in seq(min(df$raw_timestamp), max(df$raw_timestamp), window_size - overlap)) {
      ...
      sm[[length(sm) + 1]] = summarize_window(s)
    }
    sm_all <- rbind_all(sm)
    print(dim(sm_all))
    return(sm_all)
...
g1 <- ggplot(accuracies, aes(window, accuracy)) + geom_point() + 
...
g2 <- ggplot(accuracies, aes(window, accuracy)) + geom_point() + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  xlab("neural network")
...
```{r}
model_fit <- fit_rf[lapply(fit_rf, function(fit) fit$window == "win-2.0-overlap-1.5") == TRUE][[1]]$fit

testing$classe = "U"
#proccess_fn <- create_sliding_window_summary_fn(2.0, 1.5)
pred <- sapply(1:20, function(t) predict(model_fit, newdata=summarize_window(testing[t,])))
