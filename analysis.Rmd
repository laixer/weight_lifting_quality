...
window_configs = list()
for (size in seq(0.5, 5.0, 0.5)) {
  for (overlap in seq(0, size - 0.5, 0.5)) {
...
window_summaries = lapply(window_configs, function(config) {
  ...
  return(list(window=paste("win", config$size, "overlap", config$overlap, sep = "-"),
...
tune_grid_nn <- expand.grid(.decay = c(0, 1e-4, 1e-1, 1), .size = c(15, 30, 50))
